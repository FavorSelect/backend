...
const sendTicketCreationEmail = async (email, fullName, ticketNumber, subject) => {
  try {
      ...
      html: `
        <div style="background-color: #f3f4f6; padding: 40px 0; font-family: Arial, sans-serif;">
          <div style="max-width: 580px; margin: 0 auto; background-color: #ffffff; border-radius: 8px; padding: 30px; box-shadow: 0 0 10px rgba(0,0,0,0.05);">
            ...
            <p style="text-align: center; color: #555; font-size: 15px;">Our support team will contact you shortly.</p>
    ...
    console.log("Ticket creation email sent successfully:", response);
  } catch (error) {
    console.error("Error sending ticket creation email:", error);
  }
};
const sendTicketReplyEmail = async (email, fullName, ticketNumber, subject, adminReply, status) => {
  try {
    const response = await transporter.sendMail({
      from: '"FavorSelect Support" <favorselect113@gmail.com>',
      to: email,
      ...
      html: `
        <div style="background-color: #f8f9fa; padding: 40px 0; font-family: Arial, sans-serif;">
          <div style="max-width: 580px; margin: 0 auto; background-color: #ffffff; border-radius: 8px; padding: 30px; box-shadow: 0 0 10px rgba(0,0,0,0.05);">
            ...
            <p>Your support ticket <strong>#${ticketNumber}</strong> (<em>${subject}</em>) has been updated by our support team.</p>
    ...
    console.log("Ticket reply email sent:", response);
  } catch (err) {
...
module.exports = { sendTicketCreationEmail,sendTicketReplyEmail };